...
from myutils import readconfig, mymkdir
...
def calccents(tmparr):
    if shape(tmparr)[1]==0 or shape(tmparr)[0] == 0:
...
#        	weights.append(zeros(coff,float))
        ...
        print "invalid 'select' paramter (%s): use 'coadded' for standard stacking, 'lucky' for lucky, 'weighted' for weighted average exposures"%select
        return -1  
...
################creating name files that will be useful later
        ...
        listfilename = '%s/../strehl_list_%s_%d_%d_%d.dat' %(inpath,follow, gsx, gsy, rad)
        ...
        if imtype == 'CONSEC' and align == 'UNALIGN':
            ...
            if os.path.isfile(listfilename) == True:
                ...
                if mymkdir('%s/../regions/'%inpath)!= 0:
                    sys.exit()
                bpx,bpy=gsx,gsy
                nrejected = 0
                name = files[0]
                regfile = '%s/../regions/%s_%s.reg' %(inpath,name,follow)
                ...
                if maxvalue<satlevel:
                    ...
                    if len(maxindex[0]) ==1:
                        (mx,my,apf) =\
                        calccents(
                            tmpfile[0].data[534.508 : 595.508,  709.325: 770.325])
                            #bpy-rad+maxindex[0]-rad:bpy-rad+maxindex[0]+rad+1,bpx-rad+maxindex[1]-rad:bpx-rad+maxindex[1]+rad+1])
             ...
             #           print >> regf, "box(",(maxindex[1])[0]+bpx-rad+1,",",(maxindex[0])[0]+bpy-rad+1,",",2*rad+1,",",2*rad+1,",0)"
                        print >> regf, "box(",bpx-rad+1,",",bpy-rad+1,",",2*rad+1,",",2*rad+1,",0)"
                       # print "hallo"
...
##########        IF STATEMENT HERE FOR CORRELATION
        ...
        for k in xrange(coff):
            ...
            tmp2=repeat(repeat(tmp,2,axis=1),2,axis=0)
            frame=zeros((tmp2.shape[0]-4,tmp2.shape[1]-4))
            frame[::2,::2]=((3*tmp2[2:-2,2:-2]+tmp2[2:-2,1:-3]+
                             tmp2[1:-3,1:-3]+tmp2[1:-3,2:-2])/6)[::2,::2]
            frame[::2,1::2]=((3*tmp2[2:-2,2:-2]+tmp2[1:-3,2:-2]+
                              tmp2[1:-3,3:-1]+tmp2[2:-2,3:-1])/6)[::2,1::2]
            frame[1::2,1::2]=((3*tmp2[2:-2,2:-2]+tmp2[2:-2,3:-1]+
                               tmp2[3:-1,3:-1]+tmp2[3:-1,2:-2])/6)[1::2,1::2]
            frame[1::2,::2]=((3*tmp2[2:-2,2:-2]+tmp2[3:-1,2:-2]+
                              tmp2[3:-1,1:-3]+tmp2[2:-2,1:-3])/6)[1::2,::2]
...
#####     UPDATE HEADER     #####
        ...
        header.update('IMAGTYPE', '%s' % imtype, 'Consecutive, Lucky or Weighted')
