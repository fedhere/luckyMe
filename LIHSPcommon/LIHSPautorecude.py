...
yesterday = datetime.date.today() - datetime.timedelta(0,0,0)
...
for f in explist: 
   ...
   ###########parse header#############
    ...
    if USEHEADERTRGT==1:
        ...
        rashere=tra.replace(' ','')
        ras.append(rashere)
        dechere=tdec.replace(' ','')
        decs.append(dechere)
        print tname, tra, tdec
      ###########this will have to be updated for iXon 888's
    
    exps.append({'name':f, 'target':tname, 'cadence':cadence ,'gain':gain, 'bins':(hbin,vbin), 'axs':(xsize,ysize), 'ra':rashere, 'dec':dechere})
...
#                            print exps
                            expshere = [s for s in exps if (s.get('cadence')==h and s.get('gain')==g and s.get('axs')==ax and s.get('bins')==b and s.get('target')==tg and s.get('ra')==r and s.get('dec') == d )]
                                                            
                            imgshere=[s['name'] for s in expshere]

                            for i,f in enumerate(imgshere):
                                print "\n\n##PROCESSING EXPOSURES: ",f, "\n\n"
...
###of there are no images to process...
                            if len(imgshere)<=0:
                                continue
...
##############stack 5 sec of images
                                createstack(unspooledpath,0,0,1,'coadded',pc,'unalign','none',0,outpath,0,'static',2.0,SATURATE,FASTFFT)
...
##############run sextractor
                                ...
                                while (nstars>5 and dt <100 and dma < 50):
                                    ...
                                    command = 'sex -c /home/eng/LIHSPcommon/default.sex %s -CATALOG_NAME %s -DETECT_MINAREA %d -DETECT_THRESH %d'%(outfile,outpath+'/sex.cat', dma, dt)
...
##############choose target coordinates and reduce!
                                ...
                                if len(tmp.shape)>1:
                                    ...
                                    createstack(unspooledpath,stars[1,0]/2-6,stars[1,1]/2-6,60,'lucky',[0.25,0.5,1,3,5,10,50],'align','none',0,outpath,0,'static',PIXSCALE,SATURATE,FASTFFT)
                                else:
                                    stars=[tmp[1],tmp[2]]
                                    try:
                                        ...
                                        createstack(unspooledpath,stars[0]/2-6,stars[1]/2-6,60,'lucky',[0.25,0.5,1,3,5,10,50],'align','none',0,outpath,0,'static',PIXSCALE,SATURATE,FASTFFT)
                                    except:
                                        pass
                                ...
                                try:
                                    ...
                                    createstack(unspooledpath,0,0,60,'lucky',[0.25,0.5,1,3,5,10,50],'align','none',0,outpath,0,'static',PIXSCALE,SATURATE,FASTFFT)
                                except:
                                    pass
